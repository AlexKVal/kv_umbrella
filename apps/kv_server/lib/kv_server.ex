defmodule KVServer do
  ...
  def accept(port) do
    # `:binary` - data as binaries (instead of lists)
    # `packet: :line` - data line by line
    # `active: false` - blocks on `:gen_tcp.recv/2` until data is available
    # `reuseaddr: true` - allows to reuse the addr if the listener crashes
  ...
  defp write_line(_socket, {:error, :closed}) do
    # The connection was closed. It's OK.
    exit(:shutdown)
  end
